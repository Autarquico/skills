# Autarqui SkillFactory v5.1

**From Art to Engineering: A Manifesto for AI Skill Creation.**

![Autarqui SkillFactory](assets/images/01-title.png)
...
Autarqui SkillFactory is a methodology where rigor is integrated into every step of the creation process, from initial conception to final validation. It's a fundamental shift from reactive testing to proactive engineering.
...
Autarqui SkillFactory implements its philosophy through a rigorous, autonomous 4-phase architecture. This structure ensures that every skill undergoes comprehensive analysis, thorough specification, clean generation, and objective approval before it is complete.
...
Before creating anything, Autarqui SkillFactory analyzes your input to determine the best action:
...
```bash
# These all work - Autarqui SkillFactory routes automatically:

Autarqui SkillFactory: create a skill for automated code review
...
Autarqui SkillFactory is designed so skills can execute repeatable work, validate outputs, and support autonomous operation where appropriate.
...
git clone https://github.com/tripleyak/Autarqui SkillFactory.git /tmp/autarqui-skillfactory

# Codex install
cp -r /tmp/autarqui-skillfactory ~/.codex/skills/autarqui-skillfactory
rm -rf ~/.codex/skills/autarqui-skillfactory/{README.md,LICENSE,.git,.gitignore,.skillignore}

# Claude Code install
cp -r /tmp/autarqui-skillfactory ~/.claude/skills/autarqui-skillfactory
rm -rf ~/.claude/skills/autarqui-skillfactory/{README.md,LICENSE,.git,.gitignore,.skillignore}

# Or package as .skill file (respects .skillignore)
python scripts/package_skill.py /tmp/autarqui-skillfactory ./dist

# Full autonomous execution
Autarqui SkillFactory: {goal}

# Natural language activation
create skill for {purpose}

# Generate specification only
autarqui-skillfactory --plan-only
...
**Autarqui SkillFactory is a systematic methodology for quality and repeatability.**

By codifying expert analysis, rigorous specification, and multi-agent peer review into a fully autonomous system, Autarqui SkillFactory provides a blueprint for building the next generation of robust, reliable, and evolution-aware AI skills.
...
### v4.0.0
- Renamed from SkillCreator to Autarqui SkillFactory
