...
# Autarqui SkillFactory 4.1 - Intelligent Skill Router & Creator
...
**Any input works.** Autarqui SkillFactory will intelligently route to the right action:

```
# These all work - Autarqui SkillFactory figures out what you need:
...
### Creation Triggers
- `Autarqui SkillFactory: {goal}` - Full autonomous skill creation
...
- `autarqui-skillfactory --plan-only` - Generate specification without execution
...
| Command | Action |
|---------|--------|
| `Autarqui SkillFactory: {goal}` | Full autonomous execution |
| `Autarqui SkillFactory --plan-only {goal}` | Generate specification only |
| `Autarqui SkillFactory --quick {goal}` | Reduced depth (not recommended) |
| `Autarqui SkillFactory --triage {input}` | Run Phase 0 triage only |
| `Autarqui SkillFactory --improve {skill}` | Enter improvement mode for existing skill |

---

## Phase 0: Skill Triage (NEW in v4.0)

Before creating anything, Autarqui SkillFactory intelligently analyzes your input to determine the best action.
