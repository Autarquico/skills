...
When a brand from `~/.banana/brands/<name>/` is active, the skill should pass
the brand's reference images to Gemini so the output is anchored to that
visual system (logo lockup, palette, materials, lighting).

### Via the MCP server

The `@ycse/nanobanana-mcp` package exposes reference image support through
`gemini_chat` (multi-image input, up to 14 refs). To anchor a generation:

1. Open or continue a chat session referencing the brand's reference image
   paths from `~/.banana/brands/<name>/references/`.
2. Issue the crafted prompt within that session so Gemini uses the images
   as anchors.
3. Use `clear_conversation` between brands to avoid cross-contamination.

Note: `gemini_generate_image` (the one-shot tool) does not yet accept image
inputs directly. For pure one-shot brand-anchored generation, use the
fallback script below.

### Via the fallback REST script

`scripts/generate.py --brand <name>` loads `brand.yml`, encodes the listed
`reference_images` as `inlineData` parts, and sends them alongside the prompt
to `models/<model>:generateContent`. The Gemini Nano Banana API natively
accepts multi-modal input (text + images) in the `contents.parts` array --
the model uses the images as visual references during generation.

Limits and behavior:
- **Up to 14 images** per request (per Google's documented multi-image input limit).
- Images are sent inline (base64). For PNG/JPEG/WEBP/GIF the MIME type is auto-detected.
- If the request exceeds payload limits, the script truncates to the first
  N images that fit; subsequent ones are dropped with a warning.
- `referenceImages` is NOT a top-level parameter -- the API consumes them as
  generic image parts. There is no dedicated "reference" flag; the prompt's
  language ("in the style of the attached references") is what binds them.

### Prompt phrasing for visual anchors

When reference images are attached, prepend the prompt with:

> "Using the attached images as visual references for palette, materials, and
> typographic feel, render: ..."

Without that phrasing, Gemini may interpret the images as subjects to depict
rather than style anchors.
